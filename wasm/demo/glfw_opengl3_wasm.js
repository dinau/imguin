...
function findWasmBinary() {
  return locateFile("glfw_opengl3_wasm.wasm");
}
...
function __mmap_js(len, prot, flags, fd, offset, allocated, addr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(5, 0, 1, len, prot, flags, fd, offset, allocated, addr);
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(6, 0, 1, addr, len, prot, flags, fd, offset);
  offset = bigintToI53Checked(offset);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    if (prot & 2) {
...
var GLFW3 = {
  ...
  isAnyElementFocused: () => document.activeElement !== document.body,
  ...
  onKeyboardEvent(e) {
    if (e.type === "keydown") requestAnimationFrame(GLFW3.executeDeferredActions);
    if (GLFW3.fKeyboardCallback) {
      ...
      if (codepoint < 127 && e.key.length > 1) // case when eventKey is something like "Tab" (eventKey.charCodeAt(0) would be "T")
      codepoint = 0;
      var modifierBits = 0;
      if (e.shiftKey) modifierBits |= 1;
      if (e.ctrlKey) modifierBits |= 2;
      if (e.altKey) modifierBits |= 4;
      if (e.metaKey) modifierBits |= 8;
      // if(e.getModifierState('CapsLock')) modifierBits |= 0x0010;
      // if(e.getModifierState('NumLock')) modifierBits |= 0x0020;
      if (getWasmTableEntry(GLFW3.fKeyboardCallback)(GLFW3.fContext, e.type === "keydown", code, key, e.repeat, codepoint, modifierBits)) e.preventDefault();
      _free(key);
      _free(code);
    }
  },
  executeDeferredActions() {
    if (GLFW3.fDeferredActions.length > 0) {
      for (let action of GLFW3.fDeferredActions) action();
  ...
  findContext(any) {
    if (!any) return null;
    // is any a glfwWindow?
    if (GLFW3.fWindowContexts[any]) return GLFW3.fWindowContexts[any];
    // is any a canvas?
    if (any instanceof HTMLCanvasElement) return GLFW3.findContextByCanvas(any);
    // is any a selector?
  ...
  restoreCSSValues(element, ...names) {
    ...
    if (names.length === 0) names = Object.keys(properties);
    names.forEach(name => {
      const value = properties[name];
      if (!value) element.style.removeProperty(name); else element.style.setProperty(name, value);
  ...
  setCanvasResizeSelector: (any, canvasResizeSelector) => {
    ...
    if (canvasResizeSelector) {
      ...
      ctx.fCanvasResize.destroy = () => {
        for (let destructor of ctx.fCanvasResize.destructors) destructor();
      };
      if (canvasResize === window) {
        ...
        ctx.fCanvasResize.destructors.push(() => {
          GLFW3.restoreCSSValues(canvasResize);
        });
        ctx.fCanvasResize.computeSize = () => {
  ...
  makeCanvasResizable: (any, resizableSelector, handleSelector) => {
    ...
    ctx.fCanvasResize.onSizeChanged = (width, height) => {
      if (size === undefined) {
        // while not resizing (otherwise it conflicts)
  ...
  unmakeCanvasResizable: any => GLFW3.setCanvasResizeSelector(any, null)
};

function _emglfw3c_destroy() {
  ...
  for (let destructor of GLFW3.fDestructors) destructor();
  GLFW3.fContext = null;
}

function _emglfw3c_get_fullscreen_window() {
...
function _emglfw3c_get_pointer_lock_window() {
...
function _emglfw3c_init(context, scale, scaleChangeCallback, windowResizeCallback, keyboardCallback, clipboardCallback, requestFullscreen, errorHandler) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(10, 0, 1, context, scale, scaleChangeCallback, windowResizeCallback, keyboardCallback, clipboardCallback, requestFullscreen, errorHandler);
  ...
  GLFW3.fDestructors.push(() => {
    document.removeEventListener("mouseup", GLFW3.onMouseEvent);
  });
  document.addEventListener("mousedown", GLFW3.onMouseEvent);
  GLFW3.fDestructors.push(() => {
    document.removeEventListener("mousedown", GLFW3.onMouseEvent);
  });
  // handle keyboard
  document.addEventListener("keydown", GLFW3.onKeyboardEvent);
  GLFW3.fDestructors.push(() => {
    document.removeEventListener("keydown", GLFW3.onKeyboardEvent);
  });
  document.addEventListener("keyup", GLFW3.onKeyboardEvent);
  GLFW3.fDestructors.push(() => {
    document.removeEventListener("keyup", GLFW3.onKeyboardEvent);
  });
  // handle clipboard
  document.addEventListener("cut", GLFW3.onCutOrCopy);
  GLFW3.fDestructors.push(() => {
    document.removeEventListener("cut", GLFW3.onCutOrCopy);
  });
  document.addEventListener("copy", GLFW3.onCutOrCopy);
  GLFW3.fDestructors.push(() => {
    document.removeEventListener("copy", GLFW3.onCutOrCopy);
  });
  document.addEventListener("paste", GLFW3.onPaste);
  GLFW3.fDestructors.push(() => {
    document.removeEventListener("paste", GLFW3.onPaste);
  });
}

function _emglfw3c_is_any_element_focused() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(11, 0, 1);
  return GLFW3.isAnyElementFocused();
}

function _emglfw3c_is_runtime_platform_apple() {
...
function _emglfw3c_make_canvas_resizable(glfwWindow, resizableSelector, handleSelector) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0, 1, glfwWindow, resizableSelector, handleSelector);
...
var _emglfw3c_set_clipboard_string = function(content) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 0, 1, content);
...
function _emglfw3c_set_title(title) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(15, 0, 1, title);
  if (title) document.title = UTF8ToString(title);
}

function _emglfw3w_change_focus(glfwWindow, isFocussed) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(16, 0, 1, glfwWindow, isFocussed);
...
function _emglfw3w_destroy(glfwWindow) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(17, 0, 1, glfwWindow);
...
function _emglfw3w_get_computed_opacity(glfwWindow) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(18, 0, 1, glfwWindow);
  return GLFW3.fWindowContexts[glfwWindow].getComputedCSSValue("opacity");
}

function _emglfw3w_get_computed_visibility(glfwWindow) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 0, 1, glfwWindow);
  return GLFW3.fWindowContexts[glfwWindow].getComputedCSSValue("display") !== "none";
}

function _emglfw3w_get_position(glfwWindow, x, y) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(20, 0, 1, glfwWindow, x, y);
  const canvas = GLFW3.fWindowContexts[glfwWindow].canvas;
  const rect = getBoundingClientRect(canvas);
  (growMemViews(), HEAP32)[((x) >> 2)] = rect.x;
  (growMemViews(), HEAP32)[((y) >> 2)] = rect.y;
}
...
function _emglfw3w_init(glfwWindow, canvasSelector) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 0, 1, glfwWindow, canvasSelector);
  ...
  canvasCtx.originalSize = {
    width: canvas.width,
    height: canvas.height
  };
  canvasCtx.restoreCSSValue = name => {
    GLFW3.restoreCSSValues(canvas, name);
  };
  canvasCtx.restoreCSSValues = () => {
    GLFW3.restoreCSSValues(canvas);
  };
  canvasCtx.setCSSValue = (name, value) => {
    GLFW3.setCSSValue(canvas, name, value);
  };
  canvasCtx.getComputedCSSValue = name => window.getComputedStyle(canvas).getPropertyValue(name);
...
function _emglfw3w_on_created(glfwWindow) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(22, 0, 1, glfwWindow);
  if (Module.glfwOnWindowCreated) {
    Module.glfwOnWindowCreated(glfwWindow, GLFW3.fWindowContexts[glfwWindow].selector);
  }
}

function _emglfw3w_set_size(glfwWindow, width, height, fbWidth, fbHeight) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 0, 1, glfwWindow, width, height, fbWidth, fbHeight);
  const ctx = GLFW3.fWindowContexts[glfwWindow];
  const canvas = ctx.canvas;
  ...
  if (ctx.fCanvasResize && ctx.fCanvasResize.onSizeChanged) ctx.fCanvasResize.onSizeChanged(width, height);
}

function _emglfw3w_set_standard_cursor(glfwWindow, cursor) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 0, 1, glfwWindow, cursor);
  const ctx = GLFW3.fWindowContexts[glfwWindow];
  if (cursor) ctx.setCSSValue("cursor", UTF8ToString(cursor)); else ctx.restoreCSSValue("cursor");
}

function _emglfw3w_set_visibility(glfwWindow, visible) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(25, 0, 1, glfwWindow, visible);
  const ctx = GLFW3.fWindowContexts[glfwWindow];
  if (!visible) ctx.setCSSValue("display", "none"); else ctx.restoreCSSValue("display");
}
...
function _emscripten_get_screen_size(width, height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 0, 1, width, height);
  (growMemViews(), HEAP32)[((width) >> 2)] = screen.width;
  (growMemViews(), HEAP32)[((height) >> 2)] = screen.height;
}

function _emscripten_glfw3_destroy_custom_cursor(glfwCursor) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 0, 1, glfwCursor);
  delete GLFW3.fCustomCursors[glfwCursor];
}

function _emscripten_html5_remove_event_listener(target, userData, eventTypeId, callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 0, 1, target, userData, eventTypeId, callback);
  var eventHandler = {
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 0, 1, target, deferUntilInEventHandler);
  target = findEventTarget(target);
  if (!target) return -4;
  if (!target.requestPointerLock) {
    return -1;
...
function _fd_write(fd, iov, iovcnt, pnum) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(59, 0, 1, fd, iov, iovcnt, pnum);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
...
Module["requestFullscreen"] = (lockPointer, resizeCanvas) => {
  GLFW3.requestFullscreen(null, lockPointer, resizeCanvas);
};

Module["glfwGetWindow"] = any => {
  const ctx = GLFW3.findContext(any);
  return ctx ? ctx.glfwWindow : null;
};

Module["glfwGetCanvas"] = any => {
  const ctx = GLFW3.findContext(any);
  return ctx ? ctx.canvas : null;
};

Module["glfwGetCanvasSelector"] = any => {
  const ctx = GLFW3.findContext(any);
  return ctx ? ctx.selector : null;
};

Module["glfwMakeCanvasResizable"] = (any, resizableSelector, handleSelector) => {
  GLFW3.makeCanvasResizable(any, resizableSelector, handleSelector);
};

Module["glfwUnmakeCanvasResizable"] = any => {
  GLFW3.unmakeCanvasResizable(any);
};

Module["glfwRequestFullscreen"] = GLFW3.requestFullscreen;

Module["glfwIsRuntimePlatformApple"] = () => _emglfw3c_is_runtime_platform_apple();
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, ___syscall_fcntl64, ___syscall_ioctl, ___syscall_openat, __mmap_js, __munmap_js, _emglfw3c_destroy, _emglfw3c_get_fullscreen_window, _emglfw3c_get_pointer_lock_window, _emglfw3c_init, _emglfw3c_is_any_element_focused, _emglfw3c_is_runtime_platform_apple, _emglfw3c_make_canvas_resizable, _emglfw3c_set_clipboard_string, _emglfw3c_set_title, _emglfw3w_change_focus, _emglfw3w_destroy, _emglfw3w_get_computed_opacity, _emglfw3w_get_computed_visibility, _emglfw3w_get_position, _emglfw3w_init, _emglfw3w_on_created, _emglfw3w_set_size, _emglfw3w_set_standard_cursor, _emglfw3w_set_visibility, _emscripten_exit_pointerlock, _emscripten_get_device_pixel_ratio, _emscripten_get_gamepad_status, _emscripten_get_screen_size, _emscripten_glfw3_destroy_custom_cursor, _emscripten_html5_remove_event_listener, _emscripten_request_fullscreen, getCanvasSizeMainThread, setCanvasElementSizeMainThread, _emscripten_request_pointerlock, _emscripten_sample_gamepad_data, _emscripten_set_blur_callback_on_thread, _emscripten_set_focus_callback_on_thread, _emscripten_set_fullscreenchange_callback_on_thread, _emscripten_set_gamepadconnected_callback_on_thread, _emscripten_set_gamepaddisconnected_callback_on_thread, _emscripten_set_mousedown_callback_on_thread, _emscripten_set_mouseenter_callback_on_thread, _emscripten_set_mouseleave_callback_on_thread, _emscripten_set_mousemove_callback_on_thread, _emscripten_set_mouseup_callback_on_thread, _emscripten_set_pointerlockchange_callback_on_thread, _emscripten_set_pointerlockerror_callback_on_thread, _emscripten_set_touchcancel_callback_on_thread, _emscripten_set_touchend_callback_on_thread, _emscripten_set_touchmove_callback_on_thread, _emscripten_set_touchstart_callback_on_thread, _emscripten_set_wheel_callback_on_thread, _environ_get, _environ_sizes_get, _fd_close, _fd_read, _fd_seek, _fd_write ];
...
var ASM_CONSTS = {
  302376: $0 => Module.glfwGetWindow(UTF8ToString($0))
};
...
function assignWasmImports() {
  wasmImports = {
    ...
    /** @export */ emglfw3c_destroy: _emglfw3c_destroy,
    /** @export */ emglfw3c_get_fullscreen_window: _emglfw3c_get_fullscreen_window,
    /** @export */ emglfw3c_get_pointer_lock_window: _emglfw3c_get_pointer_lock_window,
    /** @export */ emglfw3c_init: _emglfw3c_init,
    /** @export */ emglfw3c_is_any_element_focused: _emglfw3c_is_any_element_focused,
    /** @export */ emglfw3c_is_runtime_platform_apple: _emglfw3c_is_runtime_platform_apple,
    /** @export */ emglfw3c_make_canvas_resizable: _emglfw3c_make_canvas_resizable,
    /** @export */ emglfw3c_set_clipboard_string: _emglfw3c_set_clipboard_string,
    /** @export */ emglfw3c_set_title: _emglfw3c_set_title,
    /** @export */ emglfw3w_change_focus: _emglfw3w_change_focus,
    /** @export */ emglfw3w_destroy: _emglfw3w_destroy,
    /** @export */ emglfw3w_get_computed_opacity: _emglfw3w_get_computed_opacity,
    /** @export */ emglfw3w_get_computed_visibility: _emglfw3w_get_computed_visibility,
    /** @export */ emglfw3w_get_position: _emglfw3w_get_position,
    /** @export */ emglfw3w_init: _emglfw3w_init,
    /** @export */ emglfw3w_on_created: _emglfw3w_on_created,
    /** @export */ emglfw3w_set_size: _emglfw3w_set_size,
    /** @export */ emglfw3w_set_standard_cursor: _emglfw3w_set_standard_cursor,
    /** @export */ emglfw3w_set_visibility: _emglfw3w_set_visibility,
    ...
    /** @export */ emscripten_glfw3_destroy_custom_cursor: _emscripten_glfw3_destroy_custom_cursor,
    /** @export */ emscripten_html5_remove_event_listener: _emscripten_html5_remove_event_listener,
